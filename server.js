...
exports.server = function (port, acceptedApiKeys, sslCert, sslKey) {
  ...
  function updateClientConn(clientConn) {
    ...
    switch (clientConn.actor) {
      ...
      default:
        log.serverAlert(
          `THIS SHOULD NEVER HAPPEN: type '${clientConn.actor}' unrecognized.`
        );
        break;
    }
  }
  ...
  function removeClient(clientConn) {
    ...
    if (!clientConn.semaphore) {
      ...
      removeClientIfRunning(clientConn) ||
      removeClientIfWaiting(clientConn)
    ) {
      found = true;
    }
    runEmitter.emit(clientConn.uid, "Requesting client closed connection.");
  ...
  app.get("/semaphore/register/", (req, res) => {
    ...
    log.semDebugEvent(
      { actor: "registrar", semaphoreName: semaphoreName, uid: client_uid },
      "creating client"
    );
  ...
  if (!Array.isArray(acceptedApiKeys)) {
    console.log(`API keys must be an array of strings.`);
    process.exit(-1);
  }
  acceptedApiKeys.forEach((key) => {
    if (!key || key.length < 10) {
      console.log(`API keys must be longer than 9 characters.`);
      process.exit(-1);
    }
  });
